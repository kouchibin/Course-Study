...
#	endif
	...
	#			endif
				...
				ev.events = EPOLLIN;
				ev.data.fd = clientfd;
				ev.data.u32 = connections.size() - 1;
				...
				if (!process_client_recv( connections[events[n].data.u32] )) {
					/* If error occured, mark connection as invalid. */
					close(events[n].data.fd);
					connections[events[n].data.u32].sock = -1;
				}
				else if (!process_client_send( connections[events[n].data.u32] )) {
