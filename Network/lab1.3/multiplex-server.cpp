...
#	endif
	...
	while( 1 )
	{
		...
		FD_SET(listenfd, &read_set); /* Listen for connections. */

		/* Setup the file descripter sets we want to monitor. */
		for (size_t i = 0; i < connections.size(); i++) {
			if (connections[i].state == eConnStateReceiving)
				FD_SET(connections[i].sock, &read_set);
			else 
				FD_SET(connections[i].sock, &write_set);
			
			/* Update max file descripter. */
			if (connections[i].sock > maxfd)
				maxfd = connections[i].sock;
		}
		

		if (select(maxfd + 1, &read_set, &write_set, &err_set, NULL) < 0) {
			perror("select() error");
			exit(-1);
		}
...
#			endif
		...
		/* Iterate through every connection to see if any actions needs to be taken. */
		for (size_t i = 0; i < connections.size(); i++) {
			int fd = connections[i].sock;

			/* If error occured, mark connection as invalid. */
			...
			if (FD_ISSET(fd, &read_set)) 
				if (!process_client_recv( connections[i] )) {
					/* If reads 0 (EOF) or error occured, mark connection as invalid. */
					close(fd);
					connections[i].sock = -1;
				}
		...
		/* Remove invalid records from connections. */
		if (connections.size()) {
			connections.erase(
				std::remove_if(
